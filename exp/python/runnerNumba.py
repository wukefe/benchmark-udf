...
def runBlkSchlsNumba():
    print('Run black-scholes with numba')
    sptprice,strike,rate,_,volatility,otime,otype,_,_ = loadData('../../data/in_1M.tbl')
    elapsed = []
    for x in range(12):
        start = time.time()
        price=BlkSchlsNumba.BlkSchls(sptprice, strike, rate, volatility, otime, otype)
        elapsed.append(time.time() - start)
    printElapsedTime(elapsed)
    print('Elapsed time (ms): %2lf (price %lf)' % (1000*numpy.mean(elapsed[2:]), numpy.sum(price)))
